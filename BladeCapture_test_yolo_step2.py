...
class BladeCapture():
    ...
    weights_path_f = '/home/iplslam/Husha/yolov5/runs/train/BladeTip/weights/best.pt'
    device = 'cuda'
    model_t = torch.hub.load('/home/iplslam/Husha/yolov5', 'custom', path=weights_path_f, source='local')
    model_t.to(device)
    model_t.eval()
    resized_width = 640
    resized_height = 640
    ...
    output_throughput = 'step2.csv'     # 変更箇所
    # target = (0, 360)   
    # target = (1920,1080)
    target = (0,0)
    # target = (0, 1080)
    ...
    def graphCut(self, img):
        ...
        try:
            ...
            # print(df_t)
            self.x1_t = int(df_t['xmin'][0])
            self.y1_t = int(df_t['ymin'][0])
            self.x2_t = int(df_t['xmax'][0])
            self.y2_t = int(df_t['ymax'][0])
